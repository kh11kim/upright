...
class Camera:
    ...
    def render(self, extrinsic):
        ...
        (w, h, rgba, depth, seg) = self.physics_client.getCameraImage(
        ...
        rgb = np.array(rgba).reshape((h, w, -1))[:,:,:3]
        #rgba = np.array(rgba).reshape((w, h, -1))
        depth = np.array(depth).reshape((w, -1))
        #rgb = rgba[:,:,:3]
        depth = (
